...
def evaluate_instance(instance_idx: int, adaptors: list, limit: int = -1, output_suffix: str = "", tree_dir: str = "out/raptor_trees"):
    ...
    # Initialize Memory (RAPTOR)
    tree_path = Path(tree_dir) / f"raptor_ttl_{instance_idx}.pkl"
    if not tree_path.exists():
        logger.error(f"RAPTOR tree not found: {tree_path} (run ingest_test_time.py first)")
        return
